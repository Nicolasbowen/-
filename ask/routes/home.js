...
exports.index = (req, res, next) => {
    let options = { sort: '-last_reply_time' };
    let query = {};
    let limit = Number(req.query.limit) || 8;
    let currentPage = Number(req.query.page) || 1;
    let startNum = (currentPage - 1) * limit + 1;
    Article.getArticleByQuery2(query, options, function(err, articles) {
        if (err) {
            return res.end(err);
        }
        let totalItem = articles.length;
        ...
        res.render('index', {
            articles: articles.slice(startNum - 1, startNum + limit - 1),
            pageInfo: pageInfo,
            title: '问题--社区问答系统',
            layout: 'indexTemplate',
        })
